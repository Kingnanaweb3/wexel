...
export function WalletActions({ onSent }: { onSent?: () => void }) {
  ...
  const [sheet, setSheet] = useState<null | "send" | "receive" | "convert">(null);

  return (
    <>
      <div style={{ display: "flex", gap: 9 }}>
        <button className="btn" disabled={!publicKey} onClick={() => setSheet("receive")}
                style={{ padding: "12px 10px", fontSize: 13.5 }}>
          Receive <ArrowDownLeft size={14} />
        </button>
        <button className="btn" disabled={!publicKey} onClick={() => setSheet("convert")}
                style={{ padding: "12px 10px", fontSize: 13.5 }}>
          Convert <Repeat size={14} />
        </button>
      ...
      {sheet === "convert" && <ConvertSheet onClose={() => setSheet(null)} onDone={onSent} />}
    </>
  );
}

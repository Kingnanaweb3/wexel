...
// A fixed palette so the allocation bar reads the same every time.
...
export default function Portfolio() {
  ...
  const crypto = total - stocks - cash;
        ...
        {publicKey && (
          <>
            ...
            <div className="card" style={{ padding: 18, marginBottom: 22 }}>
              ...
              {/* Stacked bar — one segment per holding */}
              {total > 0 && (
                <div style={{
                  ...
                  {holdings.filter(h => h.share > 0.5).map((h, i) => (
                    <div key={h.mint} style={{
                      width: `${h.share}%`, background: SWATCH[i % SWATCH.length],
                    }} />
                  ))}
                </div>
              )}
              ...
              <Split label="Crypto" value={crypto} total={total} />
            ...
            {loading && !data && <Muted>Reading your wallet…</Muted>}
            {data && holdings.length === 0 && <Muted>This wallet holds nothing yet.</Muted>}
            ...
            {holdings.length > 0 && (
              <div className="card" style={{ padding: "2px 15px" }}>
                {holdings.map((h, i) => {
                    ...
                    <>
                      <div style={{ position: "relative", flexShrink: 0 }}>
                        {h.icon
                          ...
                          : <div className="icon-chip" style={{ width: 36, height: 36, fontSize: 11 }}>
                              {h.symbol.slice(0, 3)}
                            </div>}
                        <span style={{
                  ...
                  // Stocks link through to their detail page.
                  return h.isStock
                    ? <Link key={h.mint} href={h.isPre ? `/stock/${h.symbol}?pre=1` : `/stock/${h.symbol.replace(/x$/, "")}`} style={style}>{inner}</Link>
                    : <div key={h.mint} style={style}>{inner}</div>;
                })}
              </div>
            )}
            ...
            {holdings.some(h => h.price === 0) && (
              <p style={{ fontSize: 11.5, color: "var(--faint)", lineHeight: 1.6, marginTop: 12 }}>
                Tokens marked — have no market price. On devnet that's most
                test tokens; they're shown so nothing in your wallet is hidden.
              </p>
            )}
          </>
        )}
